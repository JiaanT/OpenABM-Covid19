...
void read_param_file( parameters *params)
{
	...
	check = fscanf(parameter_file, " %i ,", &(params->centralized_quarantine_on_positive));
	if( check < 1){ print_exit("Failed to read parameter centralized_quarantine_on_positive\n"); };
